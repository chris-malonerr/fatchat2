...
describe('Rooms list screen', () => {
	...
	describe('Usage', async() => {
		it('should search room and navigate', async() => {
			...
			await element(by.id('rooms-list-view-search')).replaceText('fatchat.cat');
			await sleep(2000);
			await waitFor(element(by.id('rooms-list-view-item-fatchat.cat'))).toBeVisible().withTimeout(60000);
			await expect(element(by.id('rooms-list-view-item-fatchat.cat'))).toBeVisible();
			await element(by.id('rooms-list-view-item-fatchat.cat')).tap();
			await waitFor(element(by.id('room-view'))).toBeVisible().withTimeout(10000);
			await expect(element(by.id('room-view'))).toBeVisible();
			await waitFor(element(by.text('fatchat.cat'))).toBeVisible().withTimeout(60000);
			await expect(element(by.text('fatchat.cat'))).toBeVisible();
			await tapBack();
			await waitFor(element(by.id('rooms-list-view'))).toBeVisible().withTimeout(2000);
			await expect(element(by.id('rooms-list-view'))).toBeVisible();
			await element(by.id('rooms-list-view-search')).replaceText('');
			await sleep(2000);
			await waitFor(element(by.id('rooms-list-view-item-fatchat.cat'))).toExist().withTimeout(60000);
			await expect(element(by.id('rooms-list-view-item-fatchat.cat'))).toExist();
		});
